...
const OFFICIAL_MASTER_SCHEDULE = [
  ...
  { round: '22. Runde', tournament: '1. Division', homeTeam: 'Hillerød Fodbold', awayTeam: 'Vejle Boldklub', matchDate: '2027-03-20 19:00', finalScore: false, homeScore: null, awayScore: null },
  { round: '22. Runde', tournament: '1. Division', homeTeam: 'Vendsyssel FF', awayTeam: 'FC Fredericia', matchDate: '2027-03-20 19:00', finalScore: false, homeScore: null, awayScore: null },
  { round: '22. Runde', tournament: '1. Division', homeTeam: 'AB Gladsaxe', awayTeam: 'Hobro IK', matchDate: '2027-03-20 19:00', finalScore: false, homeScore: null, awayScore: null }
];
...
export default function AdminScreens() {
  ...
  const normalizeTeamName = (name) => {
    ...
    if (n.includes('akademisk') || n === 'ab' || n.includes('gladsaxe')) return 'ab gladsaxe';
    if (n.includes('esbjerg') || n === 'efb') return 'esbjerg fb';
    if (n.includes('vendsyssel')) return 'vendsyssel ff';
    if (n.includes('kolding')) return 'kolding if';
    if (n.includes('hvidovre')) return 'hvidovre if';
    if (n.includes('hobro')) return 'hobro ik';
  ...
  const getStadiumForTeam = (teamName) => {
    ...
    if (!stad || stad.trim() === '') {
      ...
      if (lower.includes('ab gladsaxe') || lower === 'ab' || lower.includes('akademisk')) return 'Gladsaxe Stadion';
      if (lower.includes('hillerød')) return 'Right to Dream Park';
      if (lower.includes('esbjerg') || lower === 'efb') return 'Blue Water Arena';
      if (lower.includes('kolding')) return 'Autocentralen Park';
      if (lower.includes('fredericia')) return 'Monjasa Park';
      if (lower.includes('vendsyssel')) return 'Nord Energi Arena';
      if (lower.includes('hobro')) return 'DS Arena';
      if (lower.includes('hvidovre')) return 'Pro Ventilation Arena';
      if (lower.includes('aarhus fremad')) return 'Riisvangen Stadion';
      if (lower.includes('køge')) return 'Capelli Sport Stadion';
      if (lower.includes('vejle')) return 'Vejle Stadion';
    }
    return stad || '';
  };
  ...
  if (currentScreen === 'adminMatches') {
      ...
      try {
        ...
        OFFICIAL_MASTER_SCHEDULE.forEach(official => {
          const existing = matchesList.find(m => 
            ...
            normalizeTeamName(m.awayTeam) === normalizeTeamName(official.awayTeam) && 
            normalizeRoundName(m.round) === normalizeRoundName(official.round)
          );
          ...
          if (!existing) {
            changes.push({
              ...
              action: async () => {
                const defaultStad = getStadiumForTeam(official.homeTeam);
                await db.collection('matches').add({
                  ...
                  alternativeStadium: defaultStad,
    ...
    const handlePreviewStadiumSync = () => {
      ...
      for (let m of matchesList) {
        const homeStadium = getStadiumForTeam(m.homeTeam);
    ...
    return (
      <ScrollView contentContainerStyle={styles.loginScreenContainer}>
        ...
        <TouchableOpacity style={[styles.secondaryButton, {backgroundColor: '#12352A', padding: 12, borderRadius: 8, marginBottom: 15}]} onPress={() => { 
          ...
          const defaultStad = getStadiumForTeam('AB Gladsaxe');
          setNewMatchData({ homeTeam: 'AB Gladsaxe', awayTeam: 'AaB', dateStr: '2026-07-24', timeStr: '18:00', round: '', tournament: '1. Division', alternativeStadium: defaultStad }); 
                    ...
                    <View key={m.id} style={{backgroundColor: '#fff', padding: 12, borderRadius: 8, marginBottom: 10, borderWidth: 1, borderColor: '#E5E5DF'}}>
                      <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
                        ...
                        <TouchableOpacity onPress={() => { 
                          ...
                          const matchDateObj = m.matchDate ? new Date(m.matchDate.replace(' ', 'T')) : new Date();
                          let dStr = '';
                          let tStr = '';
                          if (!isNaN(matchDateObj.getTime())) {
                            const y = matchDateObj.getFullYear();
                            const mo = String(matchDateObj.getMonth() + 1).padStart(2, '0');
                            const d = String(matchDateObj.getDate()).padStart(2, '0');
                            const h = String(matchDateObj.getHours()).padStart(2, '0');
                            const min = String(matchDateObj.getMinutes()).padStart(2, '0');
                            dStr = `${y}-${mo}-${d}`;
                            tStr = `${h}:${min}`;
                          }
                          const defaultStad = m.alternativeStadium || getStadiumForTeam(m.homeTeam);
                          setNewMatchData({ 
                            homeTeam: m.homeTeam, 
                            awayTeam: m.awayTeam, 
                            dateStr: dStr, 
                        ...
                        {Object.keys(TEAM_DB).filter(t => (TEAM_DB[t].league || '1. Division') === league).sort().map(team => (
                          <TouchableOpacity key={team} style={{padding: 15, borderBottomWidth: 1, borderColor: '#eee'}} onPress={() => { 
                            if(newMatchTeamSelector.type === 'home') {
                              const defaultStad = getStadiumForTeam(team);
                              setNewMatchData({...newMatchData, homeTeam: team, alternativeStadium: defaultStad});
                            } else {
                  ...
                  <View style={{flexDirection: 'row', gap: 10}}>
                    <View style={{flex: 1}}>
                      ...
                      <TextInput 
                        ...
                        placeholder="f.eks. 2026-07-24" 
                        value={newMatchData.dateStr} 
                        onChangeText={v => setNewMatchData({...newMatchData, dateStr: v})} 
                      />
                    </View>
                    <View style={{flex: 1}}>
                      <Text style={{fontWeight: 'bold', color: '#666', fontSize: 11, marginBottom: 4}}>Tid (HH:MM)</Text>
                      <TextInput 
                        style={styles.inputField} 
      ...
      <ScrollView contentContainerStyle={styles.loginScreenContainer}>
        ...
        <View style={{backgroundColor: '#FFFFFF', padding: 15, borderRadius: 10, marginBottom: 20, borderWidth: 1, borderColor: '#C5A059', width: '100%'}}>
          ...
          <TouchableOpacity style={styles.primaryButton} onPress={async () => {
            ...
            await db.collection('custom_teams').doc(newTeamName.trim()).set({ name: newTeamName.trim(), displayName: newTeamName.trim(), league: newTeamLeague, logo: 'https://via.placeholder.com/150', bgColor: '#12352A', color: '#FFFFFF' });
        ...
        <Modal visible={editingTeamKey !== null} transparent animationType="slide">
          <View style={styles.modalOverlay}>
            <View style={styles.modalContainer}>
              <Text style={styles.modalHeader}>REDIGER HOLD</Text>
              <Text style={{fontSize: 12, color: '#666', marginBottom: 10}}>Hold nøgle: {editingTeamKey}</Text>
              
              <Text style={{fontWeight: 'bold', color: '#666', fontSize: 11, marginBottom: 4}}>Visningsnavn (det der vises i appen)</Text>
              <TextInput style={styles.inputField} value={editTeamDisplayName} onChangeText={setEditTeamDisplayName} placeholder="F.eks. Aalborg eller AaB" />
              ...
              <TouchableOpacity style={[styles.primaryButton, {width: '100%', marginBottom: 10}]} onPress={async () => {
                if (!editTeamDisplayName.trim()) return showAlert("Fejl", "Angiv et visningsnavn.");
                const teamData = allTeamsMap[editingTeamKey] || {};
                await db.collection('custom_teams').doc(editingTeamKey).set({
                  name: editingTeamKey,
                  displayName: editTeamDisplayName.trim(),
              ...
              <TouchableOpacity style={[styles.secondaryButton, {width: '100%'}]} onPress={() => setEditingTeamKey(null)}>
                <Text style={{color: '#8A1C1C'}}>ANNULLER</Text>
              </TouchableOpacity>
            </View>
          </View>
