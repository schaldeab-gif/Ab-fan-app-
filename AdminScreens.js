      ...
      <ScrollView contentContainerStyle={styles.loginScreenContainer}>
        ...
        <TouchableOpacity style={[styles.primaryButton, {backgroundColor: '#C5A059', marginBottom: 20}]} onPress={async () => {
          ...
          try {
            ...
            Alert.alert("Succes!", "Genberegnet point og statistik for alle brugere.");
          } catch(e) { alert("Fejl: " + e.message); } finally { setIsCalculatingPoints(false); }
                  ...
                  {matchesInRound.map(m => (
                    <View key={m.id} style={{backgroundColor: '#fff', padding: 12, borderRadius: 8, marginBottom: 10, borderWidth: 1, borderColor: '#E5E5DF'}}>
                      ...
                      <TouchableOpacity onPress={async () => {
                        ...
                        if (h === null || a === null || h === '' || a === '') return alert("Indtast resultat");
                        try { await db.collection('matches').doc(m.id).update({ homeScore: parseInt(h,10), awayScore: parseInt(a,10), finalScore: true }); logActivity('resultater', `Kampresultat opdateret: ${h}-${a}`, userData?.username || 'Admin'); alert("Resultat gemt!"); } catch (error) { alert("Fejl"); }
              ...
              {newMatchTeamSelector.type ? (
                ...
                <ScrollView style={{width: '100%'}}>
                  ...
                  <TouchableOpacity style={[styles.primaryButton, {width: '100%', marginBottom: 10, marginTop: 10}]} onPress={async () => {
                    if(!newMatchData.homeTeam || !newMatchData.awayTeam || !newMatchData.round) return alert("Udfyld felter!");
                    ...
                    if(editingMatchId) { await db.collection('matches').doc(editingMatchId).update({ homeTeam: newMatchData.homeTeam, awayTeam: newMatchData.awayTeam, matchDate: finalDate, round: newMatchData.round, tournament: newMatchData.tournament, alternativeStadium: newMatchData.alternativeStadium || null }); setEditingMatchId(null); alert("Kamp opdateret!"); } 
                    else { await db.collection('matches').add({ homeTeam: newMatchData.homeTeam, awayTeam: newMatchData.awayTeam, matchDate: finalDate, round: newMatchData.round, tournament: newMatchData.tournament, alternativeStadium: newMatchData.alternativeStadium || null, finalScore: false, homeScore: null, awayScore: null, createdAt: firebase.firestore.FieldValue.serverTimestamp() }); alert("Kamp oprettet!"); }
    ...
    return (
      <ScrollView contentContainerStyle={styles.loginScreenContainer}>
        ...
        <View style={{backgroundColor: '#FFFFFF', padding: 15, borderRadius: 10, marginBottom: 20, borderWidth: 1, borderColor: '#C5A059'}}>
          ...
          <TouchableOpacity style={styles.primaryButton} onPress={async () => {
            if (!newTeamName.trim()) return alert("Angiv holdets navn.");
            ...
            setNewTeamName(''); alert(`${newTeamName} tilføjet til ${newTeamLeague}!`);
        ...
        <ScrollView contentContainerStyle={styles.loginScreenContainer}>
          ...
          <TouchableOpacity style={[styles.primaryButton, {backgroundColor: '#C5A059', marginBottom: 15}]} onPress={async () => {
            ...
            if (!result.canceled && result.assets && result.assets.length > 0) {
              setIsUploading(true); try { const response = await fetch(result.assets[0].uri); const blob = await response.blob(); const ref = storage.ref().child(`news_images/nyhed_${Date.now()}`); await ref.put(blob); setNewImage(await ref.getDownloadURL()); alert("Billede uploadet!"); } catch (error) { alert("Fejl"); } finally { setIsUploading(false); }
            }
      ...
      <ScrollView contentContainerStyle={styles.loginScreenContainer}>
        ...
        {editingSong && (
          <View style={{backgroundColor: '#FFFFFF', padding: 15, borderRadius: 10, marginBottom: 20, borderWidth: 1, borderColor: '#C5A059'}}>
            ...
            <TouchableOpacity style={styles.primaryButton} onPress={async () => { if (!editingSong) return; await db.collection('songs').doc(editingSong.id).update({ title: editingSong.title, lyrics: editingSong.lyrics, link: editingSong.link, melody: editingSong.melody || '' }); setEditingSong(null); alert("Sang opdateret!"); }}><Text style={styles.primaryButtonText}>GEM ÆNDRINGER</Text></TouchableOpacity>
        ...
        {pendingSongs.length === 0 ? <Text style={{fontStyle: 'italic', color: '#666'}}>Ingen sange afventer godkendelse.</Text> : pendingSongs.map(song => (
          <View key={song.id} style={{backgroundColor: '#FFFFFF', padding: 15, borderRadius: 8, marginBottom: 10, borderWidth: 1, borderColor: '#E5E5DF'}}>
            ...
            <View style={{flexDirection: 'row', gap: 10}}>
              <TouchableOpacity onPress={async () => { await db.collection('songs').doc(song.id).update({ approved: true }); alert("Sang godkendt!"); }} style={{backgroundColor: '#4CAF50', padding: 8, borderRadius: 4, flex: 1, alignItems: 'center'}}><Text style={{color: '#fff', fontWeight: 'bold'}}>GODKEND</Text></TouchableOpacity>
    ...
    return (
      <ScrollView contentContainerStyle={styles.loginScreenContainer}>
        ...
        <View style={{backgroundColor: '#FFFFFF', padding: 15, borderRadius: 10, marginBottom: 20, borderWidth: 1, borderColor: '#C5A059'}}>
          ...
          <TouchableOpacity style={styles.primaryButton} onPress={async () => { if (!newRssName.trim() || !newRssUrl.trim()) return alert("Udfyld felter."); await db.collection('rss_feeds').add({ name: newRssName.trim(), url: newRssUrl.trim() }); setNewRssName(''); setNewRssUrl(''); alert("Tilføjet!"); }}><Text style={styles.primaryButtonText}>TILFØJ RSS FEED</Text></TouchableOpacity>
        </View>
    ...
    return (
      <ScrollView contentContainerStyle={styles.loginScreenContainer}>
        ...
        <View style={{backgroundColor: '#FFFFFF', padding: 15, borderRadius: 10, marginBottom: 20, borderWidth: 1, borderColor: '#C5A059'}}>
          ...
          <TouchableOpacity style={styles.primaryButton} onPress={async () => {
            if (!newAwayMatchId || !newAwayInfoText.trim()) return alert("Vælg kamp og skriv info.");
            const targetMatch = matchesList.find(m => m.id === newAwayMatchId);
            if (!targetMatch) return alert("Kampen blev ikke fundet.");
            ...
            setNewAwayMatchId(null); setNewAwayInfoText(''); alert("Away Info gemt!");
